...
| `03-A股盘面信息/` | 每日盘面、板块轮动 |
| `04-投资复盘/` | 日度/月度复盘 |
| `05-行业与个股分析/` | 行业研究、个股深度、财务分析、估值模型（财务分析 / 估值模型为内嵌工具页，位于各自 `工具/` 子目录） |
| `06-投资策略分析/` | 策略回测、组合管理（策略回测为内嵌工具页，位于 `策略回测/工具/`） |
| `data/` | 文档清单与行情数据（`manifest.json` / `index-daily.json` / `macro-*.json` / `market-sentiment.json`） |
| `tools/` | 维护脚本（清单同步、小程序同步、一致性校验、行情刷新） |
| `scripts/` | 本地静态服务与文档清单生成（`serve.mjs` / `sync-manifest.mjs`） |
| `quant/` | 量化策略开发与离线验证工作区 |
| `start.bat` / `publish.bat` | 本地启动 / 一键发布到线上 |
| `miniprogram/` | 微信小程序端 |
| `archive/` | 历史临时文件归档 |
...
| 类型 | 位置 | 命名规范 |
| --- | --- | --- |
| 每日晨报 | `01-投研信息收集/每日晨报/` | `YYYY-MM-DD-晨报.pdf` + `.md` |
| 产业链周报 | `05-行业与个股分析/行业研究/` | `{赛道}产业链周报_YYYY-MM-DD.pdf` |
| 行业深度 | `05-行业与个股分析/行业研究/` | `{主题}深度投资研究报告.pdf` |

**晨报自动加载规则**：页面打开时从当天起向前探测 7 天（`MORNING_FALLBACK_DAYS = 7`），命中第一个存在的 `YYYY-MM-DD-晨报.pdf` 即内嵌展示。因此晨报文件必须严格按此命名放入 `01-投研信息收集/每日晨报/`，且停更超过 7 天页面会显示"暂无晨报"。
