# -*- coding: utf-8 -*-
"""
热点追踪量化程序 V4.7（补丁版）—— 盘前选股 + 早盘 09:32 建仓（山西证券 PTrade 版）
=====================================================================
本文件 = 热点早盘001_V4.6.py 基础上叠加"P0 + P1 全改"，仅供回测对照；
V4.4 / V4.5 / V4.6 三个版本文件均原位保留、未做任何改动。
...
T 日 09:32  buy_job
              ...
              ③ 每板块限 3 只 + 产业链簇 ≤40% + 每日新建 ≤3 笔 → order_value(..., limit_price=)
                 ★V4.7★ 限价 = min(快照×(1+2%), 护栏上限)，由 0.5% 放宽以治跳空拒单
T 日 盘中    handle_data
              ① 捕获候选日内价
              ② 移动止盈 / 分批止盈（浮盈≥12% 减至留 1 手）随时执行
              ③ ★V4.7★ 硬止损：仅当 now ≥ STOP_TIME_FLOOR(09:45) 执行；豁免最短持有期
              ④ ★V4.7★ 破均线清仓：仅当 now ≥ EXIT_BREAK_TIME(14:45) 判定（尾盘确认）
...
## 实现状态表（V4.7 逐条自检，杜绝"声称已修"）
  ...
  | ★每日新建仓上限 3 笔 / 噪声日志去重 / [净值] 打印 | ★V4.7 新增 P1-3/P2-1/P2-2★ 已实现 |
  ...
  | ★分批止盈（浮盈 12% 减半，剩余跟移动止盈） | ★V4.7 修复失效 bug★ 已实现（V4.5 新增 B-8） |
...
MAX_NEW_POSITIONS_PER_DAY = 3       # ★V4.7★ 2 → 3：V4.5 实测平均仓位利用率仅 40.4%
                                    #         （平均持仓 3.41 只/上限 6），每日 2 笔是主要瓶颈
...
TRAIL_PCT = 0.10                    # ★V4.7★ 0.07 → 0.10（用户指定）：V4.5 回测显示 6/29 那两笔
                                    #         "自高点回撤"出口是区间内唯一兑现大利润的来源
                                    #         （+40.5%/+53.5% 量级的赢仓），7% 会让它们在趋势未走完时
                                    #         提前离场；放宽到 10% 换取赢仓多跑。代价 = 回吐幅度加大。
...
def monitor_risk(context, sector_clear=False):
    ...
    for code, px in pm.items():
        ...
        if sell is None and now >= EXIT_BREAK_TIME and len(closes) >= RETREAT_MA:
            ...
            if ma and price_now < ma:
                ...
                if held_days_break >= MIN_HOLD_FOR_TIGHT_STOP:
                    sell, reason = amount, "破{}日线清仓(尾盘确认)".format(RETREAT_MA)
...
def initialize(context):
    ...
    log.info("[初始化] V4.7 启动 TRADE_ENABLED={} 调度={} 板块池={}".format(
