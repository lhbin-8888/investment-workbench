# -*- coding: utf-8 -*-
"""
热点追踪量化程序 V4.8（补丁版）—— 盘前选股 + 早盘 09:32 建仓（山西证券 PTrade 版）
=====================================================================
...
ATR_STOP_MIN_PCT = 0.05             # ★V4.8★ P0-1 0.03 → 0.05：热点票 ATR/价格常态 3~5%，
                                    #         1.8×ATR 本应给出 5.4%~9% 的止损带；旧下限 0.03
                                    #         会把低波票的止损收紧到"噪声区"内被随机扫掉。
ATR_STOP_MAX_PCT = 0.12             # ★V4.8★ P0-1 0.05 → 0.12（本版核心）：
                                    #         V4.7 实测 16 笔 ATR 止损**全部**落在 −5.0%~−10.5%、
                                    #         均值 −5.57% —— 因为 1.8×3.5%≈6.3% 被 0.05 上限
                                    #         截断，"自适应止损"退化成"固定 5% 一刀切"。
                                    #         放宽到 0.12 让它真正随波动率伸缩；单笔损失上限由
                                    #         P0-1 两段式（先减半）控制，不靠收紧阈值。
# ★V4.8★ P0-1 两段式止损：首次触发只减半，避免"盘中插针被一次全砍"
STOP_STAGED = True                  # False = 恢复"一次清仓"（旧行为）
...
PARTIAL_TAKE_PCT = 0.10             # ★V4.8★ P1-2 0.12 → 0.10：V4.7 实测 7 笔分批止盈
                                    #         **全部**落在 +12.0%~+13.0%（贴着阈值减仓），
                                    #         说明 12% 偏高、锁利时点太靠后；提前到 10% 让
                                    #         浮盈更早部分兑现（≤0 关闭该功能）
...
def initialize(context):
    ...
    log.info("[初始化] V4.8 启动 TRADE_ENABLED={} 调度={} 板块池={}".format(
    ...
    # ★V4.8★ P0-1 / P1-3 自检
    log.info("[初始化] ★V4.8 两段式止损★ 模式={} | ATR 止损带 clamp(1.8×ATR/成本, {:.0%}, {:.0%}) | "
             "首次触发减{}成、收窄至阈值×{:.0%} 撤销、同日跌破阈值×{:.1f} 或次日仍触发则清仓".format(
                 "STAGED" if STOP_STAGED else "ONE-SHOT",
                 ATR_STOP_MIN_PCT, ATR_STOP_MAX_PCT, int(STOP_STAGE1_RATIO * 10),
                 STOP_RECOVER_RATIO, STOP_STAGE1_HARD_MULT))
