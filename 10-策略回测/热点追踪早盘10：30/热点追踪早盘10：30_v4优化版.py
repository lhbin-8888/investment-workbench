# -*- coding: utf-8 -*-
"""
热点追踪量化策略 v4.0（收益增强版 · 山西证券 PTrade）
==========================================================
基线：v3.0 加固版（原文件「热点追踪早盘10：30.py」完整保留，本文件为独立优化版，二者互不覆盖）

v4.0 相对 v3.0 的六大改造（对应 v3 回测诊断结论）：
  回测事实：破EXIT_MA出场 46 笔均值 -3.72%（其中 20 笔亏超 -5%，最深 -16.7%）；
            止盈 25 笔均值 +17.29%；胜率 35.94%；8 次因高价股凑不足一手跳过买入。

【v4-1】盘中风控巡检（预计改善最大）
        风控原只在每日 10:30 执行一次，跌破止损/破线须顺延到次日 10:30 才处理。
        v4 增加 INTRADAY_RISK_TIMES（默认 8 个时点）盘中巡检，触发即走。
        兼容：日频回测（拿不到 HH:MM）自动退化为原单次执行，并打印说明。

【v4-2】大盘情绪闸门
        用「昨日涨停股今日平均溢价 + 今日强势股(涨幅>=5%)只数」判定情绪冰点，
        冰点时停买（SENTIMENT_MODE="STOP"）或预算减半（"HALF"，默认）。
        数据来源零额外成本：粗筛的日线 close 序列已含「昨日涨幅」，与当日实时涨幅同源。

【v4-3】板块共振（软过滤）
        候选按行业聚合，同行业 >= MIN_SECTOR_ALIGN 只入榜视为「真热点」：
        软模式给共振票加权（默认）、硬模式直接剔除（SECTOR_MODE="HARD"）。
        行业接口不可用或覆盖率过低时自动降级为不干预，并告警，绝不静默丢信号。

【v4-4】入场门槛收紧 + 高开防线
        量比下限 1.2 -> 1.5、成交额下限 3e8 -> 5e8、新增当日开盘涨幅上限 MAX_OPEN_PCT（防高开冲高回落）。
        （网格对比：预设 B 更严、预设 C 保持 v3 原值以隔离变量）

【v4-5】建仓预算回收（跳出「跳过即空仓」）
        高价股凑不足最小手数时，原版直接空出仓位、预算作废；v4 把该额度顺延给后续候选，
        并在 BUDGET_RECYCLE 下做最多 RECYCLE_ROUNDS 轮回填（含对已成交票加仓），直至用满预算或全员触顶。

【v4-6】弱离场 + 出场参数网格
        新增弱离场：持有 >= WEAK_EXIT_DAYS(1) 日且浮盈 <= WEAK_EXIT_PCT(0%) 即清仓
        （热点票不涨即为弱，缩短亏损持有期）。出场参数（EXIT_MA/TRAIL_PCT/MAX_HOLD_DAYS）走预设网格。
        另：所有卖出原因按类别累计统计（收盘打印），便于像上一轮那样做归因，形成迭代闭环。

参数预设（只改 PARAM_PRESET 一行，逐个跑横向对比 收益/回撤/胜率/盈亏比）：
  "A" 推荐·均衡     —— 六项全开，温和参数（默认）
  "B" 激进·快进快出 —— 入场更严（量比2.0/额8亿/涨幅收窄）+ 出场更快（EXIT_MA=2）
  "C" 隔离变量      —— 结构与风控同 A，但入场门槛保持 v3 原值（1.2 / 3亿 / 不设高开上限）
  "D" v3 基线复现   —— 六项全关、参数回 v3 原值，用于做对照回测（验证改造增益）
  "E" 单变量归因    —— 只开【v4-1】盘中巡检 +【v4-5】预算回收，用于量化这两项各自的贡献
...
# ============================================================
VERSION = "v4.0"
PARAM_PRESET = "A"             # "A"/"B"/"C"/"D"/"E"，见文件头说明；只改这一行即可切换
...
MAX_OPEN_PCT = 5.0             # 【v4-4】当日开盘涨幅上限(%)，超此视为高开过多不参与；0=不限制
...
TRAIL_PCT = 0.06               # 【v4-6】峰值回撤跟踪止盈（v3=0.08）
MAX_HOLD_DAYS = 4              # 【v4-6】时间止损（亏损票）：持有超过 N 个交易日无条件退出（v3=5）
...
# ---- 【v4-6】弱离场：热点票不涨即为弱 ----
WEAK_EXIT_ENABLED = True       # True=启用弱离场
WEAK_EXIT_DAYS = 1             # 持有 >= N 个自然日后开始判定
WEAK_EXIT_PCT = 0.0            # 浮盈 <= 该阈值即清仓（0.0=只要不赚就走；设 -0.01 可放宽一档）
...
# ---- 【v4-5】建仓预算回收 ----
BUDGET_RECYCLE = True          # True=跳单额度顺延/回填，尽量用满预算
...
_PRESETS = {
    ...
    "A": dict(
        ...
        WEAK_EXIT_ENABLED=True, WEAK_EXIT_DAYS=1, WEAK_EXIT_PCT=0.0,
        ...
        BUDGET_RECYCLE=True, TAIL_SCAN_ENABLED=False,
    ),
    # B 激进·快进快出：入场更严 + 出场更快（换手高）
    "B": dict(
        MAX_POSITIONS=5, POSITION_VALUE_RATIO=0.19, MAX_SINGLE_RATIO=0.24,
        MIN_VOL_RATIO=2.0, MIN_AMOUNT=8e8, MAX_OPEN_PCT=3.0,
        GAIN_RANGE={"main": (2.5, 3.5), "kcb": (5.0, 7.0), "cyb": (5.0, 7.0)},
        EXIT_MA=2, TRAIL_PCT=0.05, MAX_HOLD_DAYS=3,
        WEAK_EXIT_ENABLED=True, WEAK_EXIT_DAYS=1, WEAK_EXIT_PCT=0.0,
        INTRADAY_RISK_ENABLED=True, INTRADAY_RISK_TIMES=("10:00", "10:45", "11:15",
        "13:10", "13:40", "14:10", "14:40", "14:55"),
        SENTIMENT_GATE_ENABLED=True, SENTIMENT_MODE="STOP", MIN_STRONG_RATIO=0.025,
        SECTOR_MODE="SOFT", MIN_SECTOR_ALIGN=3, DETAIL_POOL=80,
        BUDGET_RECYCLE=True, TAIL_SCAN_ENABLED=False,
    ),
    # C 隔离变量：结构与风控同 A，入场门槛保持 v3 原值
    "C": dict(
        MAX_POSITIONS=6, POSITION_VALUE_RATIO=0.17, MAX_SINGLE_RATIO=0.22,
        ...
        EXIT_MA=3, TRAIL_PCT=0.06, MAX_HOLD_DAYS=4,
        WEAK_EXIT_ENABLED=True, WEAK_EXIT_DAYS=1, WEAK_EXIT_PCT=0.0,
        INTRADAY_RISK_ENABLED=True,
        SENTIMENT_GATE_ENABLED=True, SENTIMENT_MODE="HALF",
        SECTOR_MODE="SOFT", MIN_SECTOR_ALIGN=2, DETAIL_POOL=60,
        BUDGET_RECYCLE=True, TAIL_SCAN_ENABLED=False,
    ),
    # D v3 基线复现：六项全关、参数回 v3 原值（对照回测用）
    "D": dict(
        MAX_POSITIONS=5, POSITION_VALUE_RATIO=0.18, MAX_SINGLE_RATIO=0.30,
        MIN_VOL_RATIO=1.2, MIN_AMOUNT=3e8, MAX_OPEN_PCT=0.0,
        GAIN_RANGE={"main": (3.0, 4.0), "kcb": (6.0, 8.0), "cyb": (6.0, 8.0)},
        EXIT_MA=3, TRAIL_PCT=0.08, MAX_HOLD_DAYS=5,
        WEAK_EXIT_ENABLED=False, DETAIL_POOL=40,
        INTRADAY_RISK_ENABLED=False,
        SENTIMENT_GATE_ENABLED=False,
        SECTOR_MODE="OFF",
        BUDGET_RECYCLE=False, TAIL_SCAN_ENABLED=False,
    ),
    # E 单变量归因：只开【v4-1】盘中巡检 +【v4-5】预算回收
    "E": dict(
        MAX_POSITIONS=6, POSITION_VALUE_RATIO=0.17, MAX_SINGLE_RATIO=0.30,
        MIN_VOL_RATIO=1.2, MIN_AMOUNT=3e8, MAX_OPEN_PCT=0.0,
        GAIN_RANGE={"main": (3.0, 4.0), "kcb": (6.0, 8.0), "cyb": (6.0, 8.0)},
        EXIT_MA=3, TRAIL_PCT=0.08, MAX_HOLD_DAYS=5,
        WEAK_EXIT_ENABLED=False,
        INTRADAY_RISK_ENABLED=True,
        SENTIMENT_GATE_ENABLED=False,
        SECTOR_MODE="OFF", DETAIL_POOL=40,
        BUDGET_RECYCLE=True, TAIL_SCAN_ENABLED=False,
    ),
}
PRESET_APPLIED = str(PARAM_PRESET).upper() if str(PARAM_PRESET).upper() in _PRESETS else "A"
...
def _industry_of(code):
    ...
    if fn:
        ...
        for arg in (code, _pure(code)):
            try:
                v = f(arg)
                if isinstance(v, dict):
                    v = v.get(arg) or v.get(_pure(code)) or (
                        list(v.values())[0] if v else "")
    ...
    if not ind and _has("get_stock_info"):
        try:
            for k in ("industry", "industry_name", "sw_industry", "board"):
                v = get_stock_info(code, k)
                if isinstance(v, dict):
                    v = v.get(k) or (list(v.values())[0] if v else "")
                if v:
                    ind = str(v)
                    break
        except Exception:
...
def monitor_risk(context, data=None, quiet_empty=None, tag=""):
    """逐票风控。判定优先级（命中即处理下一票）：

      1) 破 EXIT_MA 日线                     → 清仓（更快出场）
      1b) 持仓峰值回落 > TRAIL_PCT            → 清仓（跟踪止盈）
      2) 破 RETREAT_MA(20) 日线               → 清仓（兜底）
      3) 保本止损（仅已减半票）                → 清仓
      4) 浮亏 <= -STOP_LOSS_PCT               → 清仓
      4b)【v4-6】弱离场：持有 >= WEAK_EXIT_DAYS 且浮盈 <= WEAK_EXIT_PCT → 清仓
      5) 时间止损（亏损票）持有 >= MAX_HOLD_DAYS → 清仓
      6) 浮盈 >= TP_FULL_PCT                  → 清仓
      7) 浮盈 >= TP_HALF_PCT 且未减半          → 减半（仅一次，写入保本价基准）
    ...
    for code, px in pm.items():
        ...
        # 4) 止损
        if rt <= -STOP_LOSS_PCT:
            if _sell_all(code, px, "止损 浮亏{:.1%}".format(rt), "止损", rt, is_new):
                _clear(code); sold_any = True
            continue
        # 4b)【v4-6】弱离场：热点票不涨即为弱
        if WEAK_EXIT_ENABLED and held_days >= WEAK_EXIT_DAYS and rt <= WEAK_EXIT_PCT:
