# -*- coding: utf-8 -*-
"""
热点追踪量化程序 V4.5（补丁版）—— 盘前选股 + 早盘 09:32 建仓（山西证券 PTrade 版）
=====================================================================
本文件 = 热点早盘001.py（V4.4）原位保留，另存为 V4.5 补丁版，仅供回测对照，
原策略文件未做任何改动。
...
T 日 盘前   before_trading_start
              ...
              ② ★信号层★ 全池批量取数（drop_today=True）→ 板块强度
                 → 三阶段 → 主线(3条) → 候选(每板块只取前2只) + [候选漏斗] 日志
       ↓ 候选只存代码与特征，不存成交价
T 日 09:31  early_job   ★原 09:35★
              ① 补卖昨日"可卖=0"的排队单
              ② 若盘前信号层失败（g.sector_state 为空）则重试一次
T 日 09:32  buy_job     ★原 10:00★
              ① 取当日进行中价 = p_now（成交基准；来源优先级 current_data →
                 handle_data 的 data → 当日 bar → 显式保守假设 T-1收盘×(1+1%)）
              ② 双护栏（仅当取到真实日内价时生效）：走弱不接刀 / 涨过头不追
              ③ 每板块限 2 只 + 产业链簇 ≤40% + 每日新建 ≤2 笔 → order_value(..., limit_price=)
T 日 盘中    handle_data        ① 捕获候选日内价  ② 每分钟止盈/止损/破10日线（不含板块清仓）
T 日 14:48  risk_fallback_job  ① ★板块级清仓唯一判定时点（当日一次）★ ② 盘内兜底风控
```

## 实现状态表（V4.5 逐条自检，杜绝"声称已修"）
  | 项 | 状态 |
  |----|------|
  | R1 可成交性优先（快照价成交 + 双护栏 + 不接刀 + 可交易性过滤） | 已实现（护栏在无日内价时明确标注"不生效"，不再假装已过滤） |
  | R2 口径清晰（T-1 完整收盘选股 + 建仓时点快照成交 + 护栏基准已收盘） | 已实现（全链路无未来函数） |
  | R3 换手预算（滚动统计 + 超预算停开新仓） | ★V4.5 修正★ 买卖双向记账 + 窗口改 20 交易日 + 每月建仓上限 |
  | R4 降级显式告警（含取数为空、下单返回空、护栏基准退化） | 已实现（新增 [口径-假设] / [口径-来源] 双日志） |
  | ★每板块最多 2 只（候选层截断 + 持仓层二次计数） | 已实现（V4 新增） |
  | ★产业链簇约束（Jaccard>0.3 同簇 ≤40%） | 已实现（V4.5 新增 B-7） |
  | ★建仓时点 09:32（run_daily 注册 + 容错窗口 + 盘内兜底） | 已实现（V4.5 由 10:00 前移） |
  | ★板块级清仓 14:48 单点判定 + 最短持有 3 日 + 破位线 | 已实现（V4.5 新增 B-1/B-2/B-3/B-4） |
  | ★分批止盈（浮盈 12% 减半，剩余跟移动止盈） | 已实现（V4.5 新增 B-8） |
...
# ---- 调度时刻（V4.5：盘前选股 + 09:31 早盘准备 + 09:32 建仓 + 14:48 板块级清仓）----
...
DIFFUSE_POSITION_RATIO = 0.15       # ★V4.6★ 0.10 → 0.15：回测窗口内候选几乎全是"扩散期"，
                                    #         半仓使单票仓位实际只剩 10%（6×10%=60% 上限，大量现金闲置）；
                                    #         且 4 笔分批止盈单全部来自扩散期持仓 → 扩散期并非无 alpha。
                                    #         若要保留"扩散期减仓"，建议设 0.12 而非 0.10。
MAX_NEW_POSITIONS_PER_DAY = 2       # ★V4.5★ 每日最多新建仓笔数（原"每天能买多少买多少"）
...
ATR_STOP_MIN_PCT = 0.03             # ★V4.6★ 0.04 → 0.03（止损收紧下限）
ATR_STOP_MAX_PCT = 0.05             # ★V4.6★ 0.08 → 0.05：0.08 叠加"次日 09:31 市价成交"后
                                    #         实测成交在 -9.2%/-9.8%/-9.3%（3 笔）→ 单笔亏损失控。
...
TRAIL_PCT = 0.07                    # ★V4.6★ 0.10 → 0.07：0.10 太松，回测中 6/29 两笔"自高点回撤 10%"
                                    #         才离场，赢仓把 1/3 的浮盈还给了市场。
...
PARTIAL_TAKE_PCT = 0.12             # 触发分批止盈的浮盈阈值（≤0 关闭该功能）
...
# ---- 产业链簇约束（★V4.5★ B-7）----
CLUSTER_JACCARD = 0.3               # 与任一持仓板块的 Jaccard 超过此值 → 视为同一产业链簇
CLUSTER_MAX_WEIGHT = 0.40           # 同一簇合计目标仓位上限（≤0 关闭该约束）
...
# ★V4.5★ B-5 修好卖出记账后，实际换手约是旧统计的 2 倍；按"目标持仓约 10 个交易日、
# 6 仓"重算，年双边约 30~40 次是"不过度交易"的合理上限（旧值 21 与真实持仓周期严重脱节）。
MAX_ANNUAL_TURNS = 30               # 滚动窗口内允许的双边换手次数（超限则停开新仓）
TURNOVER_WINDOW_DAYS = 20           # ★V4.5★ 滚动换手统计窗口改为 20 个【交易日】
...
def buy_job(context):
    ...
    base_target = total * POSITION_RATIO
    ...
    while idx < len(pending) and done < remaining and filled < daily_cap:
        ...
        # ★V4.5★ 单票目标：启动期满仓 / 扩散期半仓
        target = total * (DIFFUSE_POSITION_RATIO if stage == "扩散" else POSITION_RATIO)
    ...
    for code, px in pm.items():
        ...
        if sell is None and cost and PARTIAL_TAKE_PCT > 0 and code not in g.partial_done:
            ...
            if pr_p >= PARTIAL_TAKE_PCT:
                half = _round_lot(amount * PARTIAL_TAKE_RATIO, amount)
                ...
                if 0 < half < amount and tr_half >= half:
                    ...
                    log.info("[风控] {} 浮盈{:.1%}≥{:.0%} → 减仓 {} 股，剩余跟移动止盈".format(
                        code, pr_p, PARTIAL_TAKE_PCT, half))
                    continue
                log.info("[风控] {} 浮盈{:.1%}≥{:.0%} 但可卖量不足（{}股/需{}股），本次不减仓".format(
...
def _capture_intraday(data):
    ...
    for f in pb:
        ...
        for key in (c, _suffix(c), _canon(c)):
            ...
            px = 0.0
            try:
                v = obj.get("close") if isinstance(obj, dict) else getattr(obj, "close", None)
                px = float(v or 0)
            except Exception:
                px = 0.0
            if px > 0:
...
def initialize(context):
    ...
    log.info("[初始化] V4.5 启动 TRADE_ENABLED={} 调度={} 板块池={}".format(
