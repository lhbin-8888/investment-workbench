# -*- coding: utf-8 -*-
"""
热点追踪量化策略 v8.1（统计口径升级版 + 金额口径修复 · 山西证券 PTrade）
==========================================================
基线：v7.0 修正版（v3 / v4 / v5 / v6 / v7 五个前序文件均完整保留，本文件独立可跑，互不覆盖）

──────────────────────────────────────────────
〇、v8.0 为什么先改「统计口径」而不是改策略
──────────────────────────────────────────────
  v7 实测 +0.18%（v6 +3.23%，v3 +6.67%），但 v7 的「未加权出场累计」+658.7 却是
  v3（+249.2）的 2.6 倍 —— 指标排序与真实收益排序**完全倒挂**：

    版本    未加权出场Σ    排序      真实收益    排序
    v3        +249.2       第2        +6.67%    第1
    v6        +163.6       第3        +3.23%    第2
    v7        +658.7       第1        +0.18%    第3

  倒挂机理（金额不对称）：
    版本   买入笔数  平均单笔   赢家平均金额  输家平均金额  输家/赢家
    v3        69    20,559元    14,405元     20,575元      1.43
    v7       101    16,332元     9,903元     16,731元      1.69
  ⇒ v7 的赢家仓位只有输家的 59%（v3 为 70%）。收益率"好看"是因为仓位小，
    真金白银只拿到一半的量。**未加权均值会系统性地奖励"小仓位多笔数"的版本。**

  ⇒ 结论：继续用未加权指标定根因，等于用一把坏尺子量长度。
    v8 只做三件事，**交易逻辑一个字都不改**：
      v8-1  每笔平仓日志追加 ￥盈亏（= 卖出股数 × 持仓均价 × 浮盈），事后无需反推；
      v8-2  统计块新增「金额加权均值 / ￥合计 / 占期初本金」；
      v8-3  统计块新增「扫仓比 = 输家平均金额 / 赢家平均金额」，直接监控 1.43→1.69 这类恶化。
    另：默认预设改为 R6（行为完全等同 v6）——**跑一次即可同时完成「复核 v6 的 +3.23%」
    与「建立金额口径基线」两件事**，且可用于验证本次重构是零漂移。

──────────────────────────────────────────────
〇之二、v8.1 修复：金额口径首跑全部读到 0
──────────────────────────────────────────────
  v8.0 首跑（预设 R6，2026-09-21 21:49）结果：

    ✓ 零漂移复核**通过** —— 逐项复现 v6：
        76 笔 / 胜率 31.6% / 平均单笔 +2.15% / 未加权 +163.7%
        止盈清仓 17 笔 +20.98% ｜ 破3日线 44 笔 -2.74% ｜ 止损 10 笔 -5.53%
        破20日线 3 笔 -4.82% ｜ 时间止损 2 笔 -1.35% ｜ 止盈减半 15 笔
      截图收益 +3.23%、胜率 31.58%、盈利 24 / 亏损 52 —— 与 v6 完全相同。
      ⇒ 说明「统计口径升级」这一层重构没有引入漂移，交易逻辑与 v7 一致。

    ✗ 但新增的金额口径**全部为 0**：
        [统计-金额] 累计投入 0 元 | ￥盈亏 +0 元 | 金额加权均值 +0.00%
        [统计-金额] 赢家平均金额 0 元 | 输家平均金额 0 元 | 扫仓比 0.00 倍
        每笔平仓日志 ￥ 均为 +0（例：「[卖出] 300720.SZ 数量200 ￥-0」）

  根因（v8.1 已修）：**执行顺序错误**。
    _sell_all() 把「读持仓成本价」写在了 order_target(code, 0) **之后**。
    PTrade 回测中 order / order_target 返回时即已完成撮合并更新持仓对象，
    之后再读 cost_price 一律得到 0 → ￥盈亏 = 股数 × 0 × 浮盈 = 0。
    反证（同一份 position、同一字段，唯一差别是读取与下单的先后）：
      走「先读后下单」的 _sell_half() 完全正常 ——「止盈减半(单列) n=15 ￥+15,818」有值。
    ⇒ 判定成立。

  修复：把成本价与 ￥盈亏 的计算整体**上移到下单之前**（_sell_all 与 _sell_half 统一写法）。
        该修复只影响统计与日志，不触碰任何交易逻辑 ⇒ 零漂移性不受影响。

  新增三个预设（用于隔离从未被单独验证过的 v4-4 / v4-5）：
    "E6"  入场入口回 v3 —— 量比 1.5→1.2、成交额 5亿→3亿、细评池 60→40（v4-4 整组回退）
    "P6"  仓位组回 v3   —— 持仓上限 6→5、单票占总资产 0.17→0.18、单票上限 0.22→0.30（v4-5 整组回退）
    "EP6" 两者都回 v3
  背景：v5 / v6 / v7 三轮改的全是「出场规则」，而 v4 引入的这两组「入口 / 仓位」参数
        从未被单独隔离过；v3 与 v6 的建仓标的交集仅 43%，是四轮无一起过 v3 的最大嫌疑。

──────────────────────────────────────────────
一、v6 回测实测（40 个交易日，2026-05-06 ~ 06-30，预设 V6）
──────────────────────────────────────────────
  策略收益  +3.23%   基准 +3.58%   Alpha 0.13   Beta 0.37   夏普 0.60   最大回撤 9.12%
  胜率 31.58%        盈亏比 107.48%   盈利 24 / 亏损 52   平均持仓 2.42 天
  对照：v3 +6.67%（胜率 35.94%）｜ v4 -13.94%（17.78%）｜ v5 -6.15%（30.14%）
  → v6 比 v5 回升 9.38pct，但仍比 v3 低 3.44pct。

  净值曲线（由日志 [买入] 总资产 逐日还原，两版时点一致、可直接比较）：
    V6 在 05-12 ~ 06-09 一度领先 v3 达 +8 ~ +13.8pct（峰值 06-09：+13.8pct），
    随后在 06-11 ~ 06-30 被逐日磨平并反超：
      06-11 +11.1pct → 06-17 +4.2pct → 06-23 -0.1pct → 06-26 +4.5pct → 06-30 -1.9pct
    ⇒ v6 的四项修正（关补仓 / 取消分级 / 峰值回撤武装线 / 关板块共振）方向正确，
      残余差距不是「整段跑输」，而是被少数几笔「盘中离场打在下影线上」抹掉的。

  v6 自身统计（日志尾部）：累计平仓 76 笔，胜率 31.6%，平均单笔 +2.15%（未加权 +163.7%）
    止盈清仓 17 笔 +20.98% ｜ 退潮破3日线 44 笔 -2.74% ｜ 止损 10 笔 -5.53%
    退潮破20日线 3 笔 -4.82% ｜ 时间止损 2 笔 -1.35% ｜ 峰值回撤 0 笔（武装线生效；v5 为 11 笔）

──────────────────────────────────────────────
二、v6 残余差距归因（v3 与 v6 区间一致，逐笔配对 + 出场时点切分）
──────────────────────────────────────────────
  根因 1【盘中硬止损执行了瞬时下影线】—— 单笔代价最大的问题
    003043.SZ 华亚智能：两版同日同价买入（06-05 10:30，+3.98%）
      v3：06-10 减半 +12.5%，06-11 清仓 +28.9%
      v6：06-08 13:45 巡检触发「止损 浮亏 -5.7%」，全部卖出
    单笔差 34.6pct（300 股 × 约 70 元 ≈ 21,000 元 → 约 7,300 元 ≈ 7.3pct 净值）。
    关键：该止损由 v6 的午后巡检时点（13:45）在瞬时低点执行；v3 只在 10:30 判定，
    而 06-09 10:30 该股已回到 -5% 上方 → v3 从未触发该规则。
    另注：盘中止损并没有换来「早期保护」—— v6 最深亏损 -14.6% 出自「破3日线」，不是止损。

  根因 2【10:00 巡检时点落在开盘噪声区，跨类别系统性更差】
    v6 共 91 笔卖出行中 44 笔（48%）发生在 10:00。分类对比：
      破3日线：10:00 27 笔均值 -3.30% ｜ 10:30 及之后 17 笔均值 -1.84%
      止盈    ：10:00  9 笔均值 +14.16% ｜ 10:30 +16.86% ｜ 11:00 +19.93%
      破20日线：10:00  1 笔 -7.50% ｜ 10:30 -4.40% ｜ 13:10 -2.60%
      止损    ：10:00  5 笔 -5.58% ｜ 其余 5 笔 -5.48%（无差异 ⇒ 10:00 无额外保护价值）
    ⇒ 三个类别一致显示 10:00 出场劣于更晚时点，且其早期保护价值为 0。

  根因 3【浮盈仓位被「破均线」清仓，砍掉了右尾】（本轮不默认开启，见 v7-4）
    v6 破均线出场 47 笔中 7 笔为浮盈出场（均值 +3.10%）；其中同票配对 6 笔，
    v3 平均多赚 +3.38pct，两笔极端：
      688419.SS：v6 06-22 10:00 破3日线 卖 +7.4% ｜ v3 06-23 清仓 +21.7%（差 14.3pct）
      300939.SZ：v6 06-23 10:00 破3日线 卖 +4.6% ｜ v3 清仓 +20.3%（差 15.7pct）
    反向证据同样存在（浮亏组 40 笔：v6 均值 -3.92%，优于 v3 配对 -4.84%），
    即「破均线砍浅亏」是对的、「破均线砍浮盈」是错的 → 只对后者加限制。

  附：未隔离变量提醒 —— v6 仍沿用 v4-4 收紧后的入场门槛（量比 1.5 / 成交额 5 亿；
  v3 为 1.2 / 3 亿），两版建仓标的交集仅 43%（75 vs 66 只）。这是下一轮需要单独隔离的变量，
  本轮不动它，以免再次丧失归因能力。

──────────────────────────────────────────────
三、v7.0 修正内容（只针对「盘中离场质量」，不新增任何出场规则）
──────────────────────────────────────────────
【v7-1】巡检时点重构：删除 10:00（对应根因 2）
        INTRADAY_RISK_TIMES = 11:00 / 11:20 / 13:10 / 13:45 / 14:15 / 14:45 / 14:55
        原 10:00 的判定顺延至 10:30 主流程（只差 30 分钟），全部出场规则保留。

【v7-2】盘中「结构类出场」后移 + 盘中硬止损放宽（对应根因 1、2 的共同机理）
        把巡检时点分两类：
          结构时点 = 10:30 主流程 + 尾盘 TAIL_RISK_TIMES(14:45/14:55)
                     → 完整规则集，硬止损 -5%
          盘中时点 = 其余巡检时点
                     → ① 不判定「破3日线 / 破20日线」（均线类对盘中瞬时跌破过于敏感）；
                       ② 硬止损阈值放宽为 INTRADAY_STOP_PCT(-8%)，只兜真正的崩跌。
        止盈（+10% 减半 / +15% 清仓）与峰值回撤（武装线 +5%）在所有时点保持原样 ——
        这三项在 v6 明显优于 v3（止盈均值 +20.98% vs +17.29%），不动它们。
        ⚠️ 这与 v5 的 HARD 分级形似而实不同：v5 的失败在于「用无武装线的峰值回撤去填充 10:00」
        且「浅出场被全部禁用」；本项不新增任何深出场规则、不动止盈，只做「均线类后移 + 止损放宽」。

【v7-3】保留 v6 全部有效修正
        关补仓、峰值回撤武装线、板块共振 OFF、建仓日预算回收、情绪闸门、按原因归因统计。
        同时 INTRADAY_MA_GATE / INTRADAY_STOP_RELAX 两个开关置 False 即完全回到 v6 行为（预设 R6）。

【v7-4】可选：浮盈仓位不因破均线清仓（默认关闭，仅预设 H7 开启）
        BREAK_MA_MAX_PROFIT = 9.99（默认 = 不限制）；设为 0.01 即「破均线只用于浮亏仓位」。
        依据见根因 3，但样本仅 7 笔且由 2 笔极端案例主导，故不并入默认值，单独 A/B 验证。

──────────────────────────────────────────────
四、参数预设（只改 PARAM_PRESET 一行）
──────────────────────────────────────────────
  "V7"  默认修正版      —— v7-1 + v7-2 生效（删 10:00 + 结构类后移 + 盘中止损 -8%）
  "Z7"  只删 10:00      —— 仅 v7-1，其余与 v6 完全一致（分离「时点」变量）
  "S7"  只放宽盘中止损  —— 仅「盘中 -8% 止损」，10:00 保留、均线类仍全时点（分离「阈值」变量）
  "H7"  V7 + 浮盈不破均线 —— v7-4 开启（BREAK_MA_MAX_PROFIT=0.01）
  "R6"  v6 复现         —— 复核 +3.23%；也是金额口径的**标定样本**（已知真值）
  "D"   v3 基线复现     —— 用于复核 +6.67%
  ── v8.1 新增（以 R6 为底，只回退 v4-4 / v4-5）──
  "E6"  入场入口回 v3   —— 量比 1.5→1.2、成交额 5亿→3亿、细评池 60→40
  "P6"  仓位组回 v3     —— 持仓上限 6→5、单票占总资产 0.17→0.18、单票上限 0.22→0.30
  "EP6" 两者都回 v3

  建议跑法（一次只验一个变量）：
    第 0 步  R6（v8.1 复跑）→ ① 金额口径是否已有数 ② 金额加权能否指向 +3.23%（校准尺子）
    第 1 步  EP6 vs R6  → 把 v4-4+v4-5 一起回退，先粗测「是不是入口/仓位的问题」
    第 2 步  若 EP6 明显改善 → E6 vs R6（只回入场）｜P6 vs R6（只回仓位）→ 定位贡献方
    第 3 步  用修好的金额口径重看 V7 / Z7 / S7 / H7（旧结论全部基于未加权，需复核）
...
# ============================================================
VERSION = "v8.1"
PARAM_PRESET = "R6"            # "V7"/"Z7"/"S7"/"H7"/"R6"/"D"/"E6"/"P6"/"EP6"，见文件头说明；只改这一行即可切换
                               # 【v8】默认改为 R6：行为等同 v6，用于复核 +3.23% 并建立金额口径基线
                               # 【v8.1】保持 R6 作默认：R6 是唯一「已知真值」样本（+3.23%），
                               #   用它标定修好后的金额尺子是否指向同一结论（尺子的校准点）
...
# ---- 【v4-1】盘中风控巡检（v7-1：删除 10:00 时点）----
INTRADAY_RISK_ENABLED = True   # True=盘中按 INTRADAY_RISK_TIMES 多次跑风控
INTRADAY_RISK_TIMES = ("11:00", "11:20", "13:10", "13:45", "14:15", "14:45", "14:55")
# 注 1：10:30 由主流程统一执行（扫描+风控+建仓），此处不重复，避免重复日志与重复处理。
# 注 2：【v7-1】原 10:00 时点已删除。v6 实测：91 笔卖出行里 44 笔（48%）落在 10:00，
#   且三个类别一致显示 10:00 出场劣于更晚时点 ——
#     破3日线 -3.30%(n=27) vs 10:30 后 -1.84%(n=17)
#     止盈    +14.16%(n=9) vs 10:30 +16.86% / 11:00 +19.93%
#     破20日线 -7.50%(n=1) vs 10:30 -4.40% / 13:10 -2.60%
#   而 10:00 的早期保护价值为 0（10:00 的 5 笔止损均值 -5.58%，与其余时点 -5.48% 无差异）。
#   删除后原 10:00 的判定顺延至 10:30 主流程，全部规则保留、只差 30 分钟。

# ---- 【v7-2】盘中离场「结构类后移 + 止损放宽」----
# 两类时点：
#   结构时点 = 10:30 主流程 + 尾盘（TAIL_RISK_TIMES）→ 完整规则集，硬止损 -5%
#   盘中时点 = 其余巡检时点 → 不判「破3日线/破20日线」；硬止损放宽为 INTRADAY_STOP_PCT
# 依据：003043.SZ 单笔 —— 两版同日同价买入（06-05 +3.98%），v3 拿到 +28.9% 清仓，
#       v6 在 06-08 13:45 被「止损 浮亏 -5.7%」的盘中判定打掉，单笔差 34.6pct（约 7,300 元）。
#       v3 只在 10:30 判定，而 06-09 10:30 该股已回到 -5% 上方 → v3 从未触发。
INTRADAY_MA_GATE = True        # True=均线类出场仅在「结构时点」判定（10:30 主流程 + 尾盘）
INTRADAY_STOP_RELAX = True     # True=盘中时点的硬止损阈值放宽到 INTRADAY_STOP_PCT
TAIL_RISK_TIMES = ("14:45", "14:55")   # 尾盘结构时点：噪声已释放，执行完整规则集
INTRADAY_STOP_PCT = 0.08       # 盘中时点的硬止损阈值（10:30 与尾盘仍为 STOP_LOSS_PCT=5%）

# ---- 【v7-4】破均线只在浮亏时生效（可选，默认关闭）----
# v6 实测：破均线出场 47 笔中 7 笔为浮盈出场（均值 +3.10%），同票配对 6 笔里
#   v3 平均多赚 +3.38pct（688419 +7.4%→+21.7%、300939 +4.6%→+20.3% 两笔极端）；
#   而浮亏组 40 笔 v6 均值 -3.92% 优于 v3 配对 -4.84%，说明「砍浅亏」是对的。
# 故该项只对「浮盈仓位」加限制，默认不开启（9.99=不限制），仅预设 H7 设为 0.01。
BREAK_MA_MAX_PROFIT = 9.99     # 破均线出场仅允许在「浮盈 <= 该阈值」时触发

# 兼容保留：v5 的 HARD 分级机制（仅 R5/R0 类预设复现旧行为用；v7 默认预设下永不进入）
OPEN_NOISE_CUTOFF = ""
HARD_MODE_STOP_PCT = 0.08      # HARD 模式允许的硬止损线
...
# ============================================================
_PRESETS = {
    # ── V7 默认修正版：v7-1(删10:00) + v7-2(结构类后移 + 盘中止损 -8%) ──
    "V7": dict(
        ...
        # 板块共振保持关闭：v5 实测该功能「假生效」——420 条候选记录的板块人数全为 x1
        # （416 条「行业名==股票名」），从未发生共振，SOFT 加权对全体候选同比例施加、
        # 排序不变，只白白消耗 get_stock_info 调用。待确认平台真实行业字段来源后单独立项。
        ...
        # 尾盘结构时点（噪声已释放，执行完整规则集）
        TAIL_RISK_TIMES=("14:45", "14:55"),
        INTRADAY_RISK_ENABLED=True, INTRADAY_RISK_TIMES=("11:00", "11:20", "13:10",
        "13:45", "14:15", "14:45", "14:55"),
        INTRADAY_MA_GATE=True, INTRADAY_STOP_RELAX=True, INTRADAY_STOP_PCT=0.08,
        BREAK_MA_MAX_PROFIT=9.99,
    ),
    # ── Z7 只删 10:00（分离「时点」变量；其余与 v6 完全一致）──
    "Z7": dict(
        MAX_POSITIONS=6, POSITION_VALUE_RATIO=0.17, MAX_SINGLE_RATIO=0.22,
        MIN_VOL_RATIO=1.5, MIN_AMOUNT=5e8, MAX_OPEN_PCT=0.0,
        GAIN_RANGE={"main": (3.0, 4.0), "kcb": (6.0, 8.0), "cyb": (6.0, 8.0)},
        ...
        INTRADAY_RISK_ENABLED=True, INTRADAY_RISK_TIMES=("11:00", "11:20", "13:10",
        "13:45", "14:15", "14:45", "14:55"),
        INTRADAY_MA_GATE=False, INTRADAY_STOP_RELAX=False, INTRADAY_STOP_PCT=0.08,
        BREAK_MA_MAX_PROFIT=9.99,
    ),
    # ── S7 只放宽盘中止损（分离「阈值」变量；10:00 保留、均线类仍全时点）──
    "S7": dict(
        MAX_POSITIONS=6, POSITION_VALUE_RATIO=0.17, MAX_SINGLE_RATIO=0.22,
        MIN_VOL_RATIO=1.5, MIN_AMOUNT=5e8, MAX_OPEN_PCT=0.0,
        GAIN_RANGE={"main": (3.0, 4.0), "kcb": (6.0, 8.0), "cyb": (6.0, 8.0)},
        ...
        INTRADAY_MA_GATE=False, INTRADAY_STOP_RELAX=True, INTRADAY_STOP_PCT=0.08,
        BREAK_MA_MAX_PROFIT=9.99,
    ),
    # ── H7 = V7 + v7-4「浮盈仓位不因破均线清仓」──
    "H7": dict(
        MAX_POSITIONS=6, POSITION_VALUE_RATIO=0.17, MAX_SINGLE_RATIO=0.22,
        MIN_VOL_RATIO=1.5, MIN_AMOUNT=5e8, MAX_OPEN_PCT=0.0,
        GAIN_RANGE={"main": (3.0, 4.0), "kcb": (6.0, 8.0), "cyb": (6.0, 8.0)},
        ...
        "13:45", "14:15", "14:45", "14:55"),
        INTRADAY_MA_GATE=True, INTRADAY_STOP_RELAX=True, INTRADAY_STOP_PCT=0.08,
        BREAK_MA_MAX_PROFIT=0.01,
    ),
    # ── R6 v6 行为复现（两个开关全 False、10:00 保留）→ 用于复核 +3.23% ──
    "R6": dict(
        MAX_POSITIONS=6, POSITION_VALUE_RATIO=0.17, MAX_SINGLE_RATIO=0.22,
        MIN_VOL_RATIO=1.5, MIN_AMOUNT=5e8, MAX_OPEN_PCT=0.0,
        GAIN_RANGE={"main": (3.0, 4.0), "kcb": (6.0, 8.0), "cyb": (6.0, 8.0)},
    ...
    # ── E6 入场入口回 v3（以 R6 为底，只回退 v4-4）──
    #   量比 1.5→1.2、成交额 5亿→3亿、细评池 60→40。
    #   三者是 v3「入口」的同一组设计参数，作为整体回退（只放宽门槛而不动池子上限，
    #   会变成「更宽的粗筛 + 更宽的细评池」叠加，无法干净回答「v3 的入口是否更好」）。
    "E6": dict(
        MAX_POSITIONS=6, POSITION_VALUE_RATIO=0.17, MAX_SINGLE_RATIO=0.22,
        ...
        REINVEST_ON_SELL=False,
        SENTIMENT_GATE_ENABLED=True, SENTIMENT_MODE="HALF",
        SECTOR_MODE="OFF", MIN_SECTOR_ALIGN=2, DETAIL_POOL=40,
        BUDGET_RECYCLE=True, RECYCLE_ROUNDS=3, MAX_RECYCLE_BUYS_PER_DAY=2,
        TAIL_SCAN_ENABLED=False,
        TAIL_RISK_TIMES=("14:45", "14:55"),
        INTRADAY_RISK_ENABLED=True, INTRADAY_RISK_TIMES=("10:00", "11:00", "11:20",
    ...
    # ── P6 仓位组回 v3（以 R6 为底，只回退 v4-5）──
    #   持仓上限 6→5、单票占总资产 0.17→0.18、单票上限 0.22→0.30。
    #   三者是同一个仓位设计决策（0.17×6=1.02 总敞口 vs 0.18×5=0.90），整体回退。
    "P6": dict(
    ...
    # ── EP6 入场 + 仓位同时回 v3（= 去掉 v4-4 与 v4-5 两项 v4 遗留改动）──
    "EP6": dict(
        MAX_POSITIONS=5, POSITION_VALUE_RATIO=0.18, MAX_SINGLE_RATIO=0.30,
        MIN_VOL_RATIO=1.2, MIN_AMOUNT=3e8, MAX_OPEN_PCT=0.0,
        GAIN_RANGE={"main": (3.0, 4.0), "kcb": (6.0, 8.0), "cyb": (6.0, 8.0)},
    ...
    "D": dict(
        ...
        INTRADAY_RISK_ENABLED=False, INTRADAY_RISK_TIMES=(),
        INTRADAY_MA_GATE=False, INTRADAY_STOP_RELAX=False, INTRADAY_STOP_PCT=0.08,
        BREAK_MA_MAX_PROFIT=9.99, TAIL_RISK_TIMES=("14:45", "14:55"),
        OPEN_NOISE_CUTOFF="",
        REINVEST_ON_SELL=False,
...
PRESET_APPLIED = str(PARAM_PRESET).upper() if str(PARAM_PRESET).upper() in _PRESETS else "V7"
...
STATS = {"n": 0, "win": 0, "loss": 0, "sum": 0.0, "half_n": 0, "by_reason": {},
         # 【v8】金额口径：￥盈亏 / 赢家与输家的累计投入金额（用于算扫仓比）
         "money": 0.0, "half_money": 0.0,
         "win_money": 0.0, "win_n": 0, "loss_money": 0.0, "loss_n": 0}


def _record_trade(reason_key, rt, money=0.0, notional=0.0):
    """【v8】新增金额口径：money=该笔 ￥盈亏，notional=该笔投入金额（卖出股数 × 持仓均价）。

    未加权指标（rt 的算术平均）会被"小仓位多笔数"的版本系统性抬高；
    金额加权指标（money / notional）才是组合真正拿到的收益。
    """
    try:
        STATS["n"] += 1
        STATS["sum"] += rt
        ...
        if rt > 0:
            STATS["win"] += 1
            STATS["win_money"] += notional
            STATS["win_n"] += 1
        else:
            STATS["loss"] += 1
            STATS["loss_money"] += notional
            STATS["loss_n"] += 1
        d = STATS["by_reason"].get(reason_key)
        if d is None:
            d = [0, 0.0, 0.0, 0.0]          # n / Σrt / ￥ / Σ投入金额
...
def report_stats():
    ...
    log.info("[统计] 累计平仓 {} 笔 | 胜率 {:.1%} | 平均单笔浮盈 {:+.2%} | 累计(未加权) {:+.1%}".format(
        n, wr, avg, STATS["sum"]))
    # ---- 【v8-2】金额口径：这一块才是组合真正拿到的收益 ----
    _notional = sum(v[3] for v in STATS["by_reason"].values())
    _money = STATS["money"]
    _wavg = (_money / _notional) if _notional else 0.0
    log.info("[统计-金额] 累计投入 {:,.0f} 元 | ￥盈亏 {:+,.0f} 元 | 金额加权均值 {:+.2%} | "
             "占期初本金 {:+.2%}".format(_notional, _money, _wavg, _money / 100000.0))
    # ---- 【v8-3】扫仓比：输家平均金额 / 赢家平均金额 ----
    _wa = (STATS["win_money"] / float(STATS["win_n"])) if STATS["win_n"] else 0.0
    _la = (STATS["loss_money"] / float(STATS["loss_n"])) if STATS["loss_n"] else 0.0
    log.info("[统计-金额] 赢家平均金额 {:,.0f} 元(n={}) | 输家平均金额 {:,.0f} 元(n={}) | "
             "扫仓比(输/赢) {:.2f} 倍 ← 越大越危险（v3=1.43 / v7=1.69）".format(
                 _wa, STATS["win_n"], _la, STATS["loss_n"], (_la / _wa) if _wa else 0.0))
    for k in sorted(STATS["by_reason"].keys(), key=lambda x: -STATS["by_reason"][x][0]):
        c, s, m, nt = STATS["by_reason"][k]
        log.info("       {:<18} n={:<3} 均值{:+.2%} 累计{:+.1%} | 金额加权{:+.2%} ￥{:+,.0f}".format(
            k, c, s / float(c), s, (m / nt) if nt else 0.0, m))
    if STATS.get("half_n"):
        log.info("       {:<18} n={:<3} （减半不计入平仓笔数） ￥{:+,.0f}".format(
            "止盈减半(单列)", STATS["half_n"], STATS["half_money"]))
...
def _sell_all(code, px, reason, reason_key="", rt=0.0, entry_today=False):
    ...
    if enable <= 0:
        ...
        if entry_today:
            ...
            log.error("[卖出失败] " + msg)
        return False
    # 【v8.1 关键修复】成本价必须在**下单之前**取！
    #   PTrade 回测中 order / order_target 返回时即已完成撮合并更新持仓对象，
    #   之后再读 cost_price 一律得到 0 → ￥盈亏 = 股数 × 0 × 浮盈 = 0，
    #   进而使「累计投入 / 金额加权 / 扫仓比」全部归零（v8.0 首跑实测）。
    #   反证：走「先读后下单」的 _sell_half() 完全正常（n=15 ￥+15,818）。
    _cost = float(_pos_field(px, _COST_FIELDS) or 0)
    _nt = enable * _cost
    _mn = _nt * rt
    if not TRADE_ENABLED:
        log.info("[信号] 应清仓 {} 数量{} ￥{:+,.0f} —— {}".format(code, enable, _mn, reason))
        _record_trade(reason_key or "信号", rt, _mn, _nt)
        return True
    try:
        if _has("order_target"):
            ...
            order(code, -enable)
        log.info("[卖出] {} 数量{} ￥{:+,.0f} —— {}".format(code, enable, _mn, reason))
        _record_trade(reason_key or "其它", rt, _mn, _nt)
        return True
    except Exception as e:
...
def _sell_half(code, px, reason, reason_key="止盈减半", rt=0.0):
    ...
    if qty <= 0:
        ...
        return False
    # 【v8-1/v8.1】减半的 ￥盈亏 同样单独记账（不并入平仓笔数，避免污染胜率与均值）
    #   顺序要点：必须在本函数下方的 order() 之前取成本价 —— v8.0 的清仓路径正是
    #   把这一步放到了下单之后才归零。此处顺序本就正确，保留作对照。
    _cost_h = float(_pos_field(px, _COST_FIELDS) or 0)
    _mn_h = qty * _cost_h * rt
    if not TRADE_ENABLED:
        log.info("[信号] 应减半 {} 卖{}股 ￥{:+,.0f} —— {}".format(code, qty, _mn_h, reason))
        STATS["half_n"] += 1
        STATS["half_money"] += _mn_h
        return True
    try:
        if _has("order"):
            order(code, -qty)
        ...
        log.info("[卖出] {} 减半卖{}股 ￥{:+,.0f} —— {}".format(code, qty, _mn_h, reason))
        STATS["half_n"] += 1
        STATS["half_money"] += _mn_h
        return True
    except Exception as e:
...
def monitor_risk(context, data=None, quiet_empty=None, tag="", stage="NORMAL"):
    ...
    【v6-2 / v7-2 重要变更】本函数现按「规则性质」而非「时间早晚」分级：
      结构时点（stage="NORMAL"，即 10:30 主流程与 14:45/14:55 尾盘）→ 完整规则集，硬止损 -5%
      盘中时点（stage="INTRADAY"，即 11:00/11:20/13:10/13:45/14:15）→
          ① INTRADAY_MA_GATE=True 时不判定「破3日线 / 破20日线」（均线对盘中瞬时跌破过敏）；
          ② INTRADAY_STOP_RELAX=True 时硬止损阈值放宽为 INTRADAY_STOP_PCT(-8%)，只兜崩跌。
      止盈（+10% 减半 / +15% 清仓）与峰值回撤（武装线 +5%）在所有时点保持原样。

      历史教训：v5 曾在 10:00 进入 HARD 模式（禁掉所有浅出场、保留无武装线的峰值回撤、
      并把止损放宽到 -8%）→ 开盘半小时只剩「深亏出场」，11 笔峰值回撤均值 -7.02%。
      本项与 v5 的差别：不新增任何深出场规则、不动止盈、浅出场只是「后移到结构时点」而非禁用。
      stage="HARD" 仅保留给 R5/R0 类预设复现 v5/v4 旧行为。
    ...
    _stage = str(stage).upper()
    _hard = (_stage == "HARD")                  # 仅 R5/R0 类预设复现旧行为
    _intraday = _hard or (_stage == "INTRADAY")  # 【v7-2】盘中时点
    # 【v7-2】均线类出场是否允许在「此刻」判定：结构时点恒可用；
    #   盘中时点仅在 INTRADAY_MA_GATE=False 时可用（预设 S7/R6 用于分离变量）。
    # 【v7-4】再加一道「浮盈上限」：BREAK_MA_MAX_PROFIT 默认为 9.99（不限制），
    #   设为 0.01 即「破均线只用于浮亏仓位」。
    _ma_gate_ok = (not _intraday) or (not INTRADAY_MA_GATE)
    for code, px in pm.items():
        ...
        # 1) 破 EXIT_MA 日线（【v7-2】盘中时点后移；【v7-4】浮盈上限）
        if _ma_gate_ok and rt <= BREAK_MA_MAX_PROFIT and ma_exit and cur < ma_exit:
        ...
        # 2) 破 20 日线（【v7-2】盘中时点后移；【v7-4】浮盈上限）
        if _ma_gate_ok and rt <= BREAK_MA_MAX_PROFIT and ma20 and cur < ma20:
        ...
        if (not _hard) and BREAKEVEN_STOP and halved.get(code) and code in breakeven \
            ...
            continue
        # 4) 止损
        #   【v7-2】盘中时点放宽到 INTRADAY_STOP_PCT(-8%)：v6 实测盘中止损未提供早期保护
        #   （最深亏损 -14.6% 出自破3日线，不是止损），却在 003043.SZ 上把一笔最终 +28.9% 的
        #   持仓以 -5.7% 打掉（单笔差 34.6pct）。结构时点（10:30/尾盘）仍按 -5% 严格执行。
        if _hard:
            _stop_line, _stop_tag, _stop_key = HARD_MODE_STOP_PCT, "硬止损(噪声区放宽)", "止损"
        elif _intraday and INTRADAY_STOP_RELAX:
            _stop_line, _stop_tag, _stop_key = (
                INTRADAY_STOP_PCT,
                "止损(盘中放宽至{:.0%})".format(INTRADAY_STOP_PCT), "止损(盘中)")
        else:
            _stop_line, _stop_tag, _stop_key = STOP_LOSS_PCT, "止损", "止损"
        if rt <= -_stop_line:
            _rsn = "{} 浮亏{:.1%}".format(_stop_tag, rt)
            if _sell_all(code, px, _rsn, _stop_key, rt, is_new):
                _clear(code); sold_any = True
            continue
...
def initialize(context):
    ...
    _has10 = ("10:00" in tuple(INTRADAY_RISK_TIMES or ()))
    if not INTRADAY_RISK_ENABLED:
        _t1 = "盘中巡检关闭（v3 基线）"
    elif _has10:
        _t1 = "含 10:00 · v6 复现"
    else:
        _t1 = "10:00 已删"
    log.info("[初始化] v7 修正开关：[v7-1]巡检时点={}个（{}） "
             "[v7-2]盘中均线后移={} 盘中止损放宽={} "
             "[v7-4]破均线浮盈上限={}".format(
                 len(INTRADAY_RISK_TIMES), _t1,
                 "开" if INTRADAY_MA_GATE else "关",
                 "开(-{:.0%})".format(INTRADAY_STOP_PCT) if INTRADAY_STOP_RELAX
                 else "关(仍按 -{:.0%})".format(STOP_LOSS_PCT),
                 ("不限" if BREAK_MA_MAX_PROFIT > 1 else "浮盈<={:.0%}".format(BREAK_MA_MAX_PROFIT))))
    log.info("[初始化] v6 保留开关：[v6-1]盘后补仓={} [v6-3]峰值回撤武装线={} "
    ...
    log.info("[初始化] v8.1 统计口径（金额字段已修复）：每笔平仓打印 ￥盈亏；"
             "统计块含「金额加权 / ￥合计 / 扫仓比」"
             "（未加权指标已被证实会与真实收益倒挂，不作为决策依据）")
    _fb = []
    if PRESET_APPLIED in ("E6", "EP6"):
        _fb.append("入场入口回 v3(量比{:.1f}/额{:.0f}亿/池{})".format(
            MIN_VOL_RATIO, MIN_AMOUNT / 1e8, DETAIL_POOL))
    if PRESET_APPLIED in ("P6", "EP6"):
        _fb.append("仓位组回 v3(上限{}只/单票占比{:.2f}/单票上限{:.2f})".format(
            MAX_POSITIONS, POSITION_VALUE_RATIO, MAX_SINGLE_RATIO))
    if _fb:
        log.info("[初始化] 本预设相对 R6 的回退项：{}".format(" + ".join(_fb)))
    ...
    if INTRADAY_RISK_ENABLED and INTRADAY_RISK_TIMES:
        ...
        _tail = tuple(TAIL_RISK_TIMES or ())

        def _stage_of(_t):
            if _cut and str(_t) < _cut:
                return "HARD(仅硬止损/止盈)"
            if str(_t) in _tail:
                return "结构时点(全量,止损-{:.0%})".format(STOP_LOSS_PCT)
            if INTRADAY_MA_GATE or INTRADAY_STOP_RELAX:
                return "盘中(均线{}止损-{:.0%})".format(
                    "后移" if INTRADAY_MA_GATE else "可用",
                    INTRADAY_STOP_PCT if INTRADAY_STOP_RELAX else STOP_LOSS_PCT)
            return "全量(等同 v6)"
        log.info("[初始化] 巡检时点分级：{}".format("，".join(
            "{}→{}".format(_t, _stage_of(_t)) for _t in INTRADAY_RISK_TIMES)))
    _diag_api()
...
def _risk_watch(context, data, hhmm):
    """【v4-1 / v7-2】盘中风控巡检：(日期, 时点) 幂等，只跑风控不下单。

    时点分级（v7-2，按「规则性质」而非「时间早晚」）：
      结构时点 = 尾盘 TAIL_RISK_TIMES(14:45/14:55) → stage=NORMAL，完整规则集、硬止损 -5%
      盘中时点 = 其余巡检时点                      → stage=INTRADAY，均线类后移 + 止损放宽 -8%
    两个开关（INTRADAY_MA_GATE / INTRADAY_STOP_RELAX）全为 False 时，
    所有时点回落到 NORMAL，行为等同 v6（预设 R6 即为此用途）。
    OPEN_NOISE_CUTOFF 仅在 R5/R0 类复现预设下生效（HARD 模式）。
    """
    today = _context_date(context)
    ...
    seen.add(key)
    _cut = str(OPEN_NOISE_CUTOFF or "")
    if _cut and str(hhmm) < _cut:
        stage, _mark = "HARD", "(hard)"
    elif str(hhmm) in tuple(TAIL_RISK_TIMES or ()):
        stage, _mark = "NORMAL", "(structure)"
    else:
        stage = "INTRADAY" if (INTRADAY_MA_GATE or INTRADAY_STOP_RELAX) else "NORMAL"
        _mark = "(intraday)" if stage == "INTRADAY" else ""
    try:
        monitor_risk(context, data,
                     tag="巡检{}{}".format(hhmm, _mark),
                     stage=stage)
    except Exception as e:
