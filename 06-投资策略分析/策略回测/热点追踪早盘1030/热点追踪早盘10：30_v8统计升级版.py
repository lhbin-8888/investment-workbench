# -*- coding: utf-8 -*-
"""
热点追踪量化策略 v8.0（统计口径升级版 · 山西证券 PTrade）
==========================================================
...
──────────────────────────────────────────────
  ...
  "R6"  v6 复现         —— 用于在同一次回测环境复核 +3.23%
  "D"   v3 基线复现     —— 用于复核 +6.67%

  建议跑法（一次只验一个变量）：
    第 1 步  R6 vs V7  → V7 是否改善；改善则进第 2 步定位
    第 2 步  V7 vs Z7  → 「删 10:00」贡献多少
    第 3 步  V7 vs S7  → 「盘中止损放宽」贡献多少
    第 4 步  V7 vs H7  → 「浮盈不破均线」是否值得开（与前两项可能重叠）
...
# ============================================================
VERSION = "v8.0"
PARAM_PRESET = "R6"            # "V7"/"Z7"/"S7"/"H7"/"R6"/"D"，见文件头说明；只改这一行即可切换
...
def _sell_all(code, px, reason, reason_key="", rt=0.0, entry_today=False):
    ...
    if not TRADE_ENABLED:
        ...
        _record_trade(reason_key or "信号", rt, enable * float(_pos_field(px, _COST_FIELDS) or 0) * rt,
                      enable * float(_pos_field(px, _COST_FIELDS) or 0))
        return True
    try:
...
def _sell_half(code, px, reason, reason_key="止盈减半", rt=0.0):
    ...
    if qty <= 0:
        ...
        return False
    # 【v8-1】减半的 ￥盈亏 同样单独记账（不并入平仓笔数，避免污染胜率与均值）
...
def initialize(context):
    ...
    log.info("[初始化] v8 统计口径：每笔平仓打印 ￥盈亏；统计块含「金额加权 / ￥合计 / 扫仓比」"
