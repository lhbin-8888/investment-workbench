# -*- coding: utf-8 -*-
"""
热点追踪量化策略 v5.0（修正版 · 山西证券 PTrade）
==========================================================
基线：v4.0 优化版（「热点追踪早盘10：30_v4优化版.py」与 v3「热点追踪早盘10：30.py」均完整保留）

──────────────────────────────────────────────
一、v4 回测实测（40 个交易日，2026-05-06 ~ 06-30，预设 A）
──────────────────────────────────────────────
  策略收益 -13.94%   基准 +3.58%   最大回撤 21.74%
  胜率 17.78%        盈亏比 65.42%   平均持仓 1.42 天   平仓 90 笔
  对照 v3：收益 +6.67%、胜率 35.94%、平均持仓 3.94 天、建仓 69 笔
  → 结论：v4 六大改造净贡献为负，收益 -20.6pct、胜率 -18.2pct、换手 +58%。

  按原因归因（v4 日志自带统计）：
    弱离场        n=47   均值 -1.79%   累计 -83.9%   胜率  0.0%  ← 头号失血点
    退潮破3日线    n=23   均值 -3.18%   累计 -73.2%   胜率 21.7%
    止损          n=6    均值 -5.51%   累计 -33.0%
    峰值回撤      n=4    均值 -1.28%   累计  -5.1%
    止盈           n=20   均值 +17.04%  累计 +340.8%  胜率 100%   ← 唯一正贡献

  三项根因（v5 据此修正）：
  (1)【弱离场过激】规则为「持有>=1自然日 且 浮盈<=0 即清仓」，且巡检首个时点 10:00 执行。
     实际含义 = 买入次日开盘半小时内只要不赚就砍 → 47 笔中 37 笔「持有1日」、30 笔发生在 10:00，
     胜率 0%。开盘 30 分钟是全天噪声最大区间，该规则把正常震荡系统性卖在局部低点。
  (2)【高开上限误杀龙头】MAX_OPEN_PCT=5% 在 40 天内过滤 261 次，其中含开盘 +13.75%、+11.66%
     等当周主线龙头。热点策略的核心猎物就是「高开突破」，一刀切掉后候选池退化为「弱势补涨票」，
     直接解释胜率从 35.94% → 17.78% 的塌陷。
  (3)【巡检时点选在噪声区】10:00 全量判定，使「破3日线」卖点从原策略的 10:30 前移到 10:00，
     23 笔破线出场中 19 笔集中在 10:00，多数为低开噪声误伤。

  另：板块共振实际未生效（_industry_of 取不到行业 → 全部落入空字符串同一桶，
  sector_cnt 恒等于候选总数），436 次无效调用；情绪闸门 39 次全为「放行」，区间内空转。

──────────────────────────────────────────────
二、v5.0 修正内容（做减法 + 外科手术式修正）
──────────────────────────────────────────────
【v5-1】弱离场默认关闭
        WEAK_EXIT_MODE="OFF"。该项是 v4 亏损的最大来源（-83.9%、胜率0%），
        且与既有的「破EXIT_MA日线」判断高度冗余，无需再叠加。
        保留开关，如需回测可设 "CONFIRM"（T+2 且浮亏<=-2% 才砍）。

【v5-2】取消高开上限
        MAX_OPEN_PCT=0.0（不限制）。强势股的确认改由既有的「回踩过滤」承担：
        要求 现价 >= 当日开盘价（跌破开盘价视为动量转弱，已有实现）。
        「高开 + 现价站在开盘价上方」正是热点延续的形态，不应过滤。

【v5-3】巡检时点分级（噪声隔离）
        新增 OPEN_NOISE_CUTOFF="10:30"：早于该时点的巡检进入 HARD 模式，
        只保留「峰值回撤 / 硬止损 / 止盈清仓 / 止盈减半」这四类确定性判断，
        不做 破均线 / 弱离场 / 时间止损 / 保本止损 —— 把均线类判断交回 10:30 主流程
        及 11:00 之后的巡检，避免在开盘噪声区误杀。
        效果：尾部亏损保护保留（v4 已把最深亏损从 -16.7% 收到 -5.5%，这是 v4 唯一被验证的
        正贡献，必须保住），同时消除 10:00 的系统性误卖。

【v5-4】板块共振接口修复
        _industry_of 增加 get_stock_info 的多签名深解析（列表字段 / 字符串字段 / 无参全量）；
        仍取不到行业时自动把 SECTOR_MODE 降级为 "OFF" 并只告警一次，不再做无效调用。

【v5-5】预算回收限速
        保留顺延与回填（原 v3 有 8 次因高价股凑不足一手而空仓），
        但新增 MAX_RECYCLE_BUYS_PER_DAY=2：每日补建仓不超过 2 笔，防止
        「砍仓 → 立即买新票 → 次日再砍」的高频轮动闭环。

【v5-6】出场参数回归 v3 口径
        TRAIL_PCT 6%→8%、MAX_HOLD_DAYS 4→5（v4 的收紧无正贡献：
        峰值回撤 4 笔均值 -1.28%，属噪声触发）。EXIT_MA 保持 3 不变。

【v5-7】保留项
        盘中风控巡检（改造为分级模式，见 v5-3）、情绪闸门（区间内空转但无害，保留待长周期验证）、
        高开/回踩过滤中的回踩部分、按原因归因的平仓统计。

──────────────────────────────────────────────
三、参数预设（只改 PARAM_PRESET 一行）
──────────────────────────────────────────────
  "V5" 修正版（默认）—— 上述 v5-1 ~ v5-6 全部生效，与 v4 预设 A 形成 A/B 对照
  "R0" v4 预设 A 复现 —— 用于在同一次回测环境下复核 -13.94% 这个基线
  "D"  v3 基线复现    —— 六项改造全关、参数回 v3 原值

  建议跑法：先跑 V5（默认）与已有的 -13.94% / +6.67% 对比；
  若 V5 转正，再逐步回开单项（改 PARAM_PRESET 或改参数）定位每项贡献。
...
# ============================================================
VERSION = "v5.0"
PARAM_PRESET = "V5"            # "V5"/"R0"/"D"，见文件头说明；只改这一行即可切换
...
# 【v5-3】早于下列时点的巡检进入 HARD 模式：只做 峰值回撤/硬止损/止盈，不做 破均线/弱离场/时间止损
#        理由：v4 的 10:00 全量判定使破线卖点前移到开盘噪声区（23 笔破线中 19 笔集中在 10:00）
OPEN_NOISE_CUTOFF = "10:30"
HARD_MODE_STOP_PCT = 0.08      # HARD 模式允许的硬止损线（比常规 -5% 更宽，避免噪声误杀，只兜住深亏尾部）
...
# ---- 【v4-1 延伸】满仓时卖出释放额度当日是否立即补仓 ----
REINVEST_ON_SELL = True        # True=当日风控卖出释放的额度当日可用于建仓（已在主流程顺序上保证）
...
_PRESETS = {
    ...
    "V5": dict(
        ...
        EXIT_MA=3, TRAIL_PCT=0.08, MAX_HOLD_DAYS=5,
    ...
    "R0": dict(
        ...
        GAIN_RANGE={"main": (3.0, 4.0), "kcb": (6.0, 8.0), "cyb": (6.0, 8.0)},
        EXIT_MA=3, TRAIL_PCT=0.06, MAX_HOLD_DAYS=4,
...
PRESET_APPLIED = str(PARAM_PRESET).upper() if str(PARAM_PRESET).upper() in _PRESETS else "V5"
...
def monitor_risk(context, data=None, quiet_empty=None, tag="", stage="NORMAL"):
    """逐票风控。判定优先级（命中即处理下一票）：
      ...
      1) 破 EXIT_MA 日线                     → 清仓（更快出场）        [NORMAL]
      1b) 持仓峰值回落 > TRAIL_PCT            → 清仓（跟踪止盈）        [两级都跑]
      2) 破 RETREAT_MA(20) 日线               → 清仓（兜底）            [NORMAL]
      3) 保本止损（仅已减半票）                → 清仓                    [NORMAL]
      4) 浮亏 <= -STOP_LOSS_PCT               → 清仓                    [两级都跑]
      4b)【v5-1】弱离场（默认 OFF）            → 清仓                    [NORMAL]
      5) 时间止损（亏损票）持有 >= MAX_HOLD_DAYS → 清仓                  [NORMAL]
      6) 浮盈 >= TP_FULL_PCT                  → 清仓                    [两级都跑]
      7) 浮盈 >= TP_HALF_PCT 且未减半          → 减半（仅一次）           [两级都跑]

    盘中巡检（【v4-1】）会多次调用本函数，靠 halved / entry_date / 幂等标记防重复处理。

    【v5-3】stage="HARD"（开盘噪声区专用，见 OPEN_NOISE_CUTOFF）：
      只保留 峰值回撤 / 硬止损 / 止盈清仓 / 止盈减半 四类确定性判断，
      跳过 破均线 / 弱离场 / 时间止损 / 保本止损 —— 避免在开盘 30 分钟的高波动区
      被低开噪声触发均线类误卖（v4 的 10:00 全量判定即为此类误杀：23 笔破线中 19 笔在 10:00）。
      硬止损线同时放宽到 HARD_MODE_STOP_PCT（默认 -8%），只兜住深亏尾部。
    """
    ...
    for code, px in pm.items():
        ...
        if (not _hard) and ma_exit and cur < ma_exit:
            ...
            continue
        # 1b) 峰值回撤（两级都跑：这是确定性事实，不受噪声影响）
        if TRAIL_PCT > 0 and peak.get(code) and cur <= peak[code] * (1 - TRAIL_PCT):
            if _sell_all(code, px, "峰值回撤{:.0%} 浮盈{:.1%}".format(TRAIL_PCT, rt),
                         "峰值回撤", rt, is_new):
                _clear(code); sold_any = True
            continue
...
def initialize(context):
    ...
    log.info("[初始化] v5 修正开关：[v5-1]弱离场={} [v5-2]高开上限={} "
             "[v5-3]噪声区隔离（{}前只做硬止损/止盈） [v5-4]板块共振={}(>= {}只) "
             "[v5-5]回收限速={}/日 [v5-6]EXIT_MA={} TRAIL={:.0%} 时间止损={}日".format(
                 str(WEAK_EXIT_MODE).upper(),
                 "不限" if MAX_OPEN_PCT <= 0 else MAX_OPEN_PCT,
                 (str(OPEN_NOISE_CUTOFF) if OPEN_NOISE_CUTOFF else "关"),
                 SECTOR_MODE, MIN_SECTOR_ALIGN,
                 (MAX_RECYCLE_BUYS_PER_DAY if MAX_RECYCLE_BUYS_PER_DAY > 0 else "不限"),
                 EXIT_MA, TRAIL_PCT, MAX_HOLD_DAYS))
    log.info("[初始化] 保留开关：[v4-1]盘中巡检={}（{}个时点） [v4-2]情绪闸门={}({}) "
             "量比{} 额{:.0f}亿 预算回收={} 持仓上限{}只".format(
...
def _risk_watch(context, data, hhmm):
    """【v4-1 / v5-3】盘中风控巡检：(日期, 时点) 幂等，只跑风控不下单。

    【v5-3】早于 OPEN_NOISE_CUTOFF（默认 10:30）的时点进入 HARD 模式：
    只做 峰值回撤 / 硬止损 / 止盈 四类确定性判断，均线类判断交回 10:30 主流程及之后时点。
    """
    today = _context_date(context)
