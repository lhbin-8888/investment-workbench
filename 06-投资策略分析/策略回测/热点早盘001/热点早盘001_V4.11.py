# -*- coding: utf-8 -*-
"""
热点追踪量化程序 V4.11（补丁版）—— 盘前选股 + 早盘 09:32 建仓（山西证券 PTrade 版）
=====================================================================
V4.11 = V4.10 的**纯 bug 修复版**（未动任何策略参数，便于下次回测干净隔离效果）。
依据：《热点早盘001_V4.10-回测归因与结论.md》—— V4.10 实跑 +7.01%/回撤4.65%（V4.9 −4.21%），
但日志暴露两个"代码与日志不一致"的问题：

  ★V4.11 P0-1（真 bug）★ 赢家峰值追踪失效
    monitor_risk 里 `g.winner_peak_pnl.get(code, _pr_now)` 把缺省值取成了**当前浮盈**，
    首次进入时 `_pr_now > _pk_prev` 恒 False → 峰值键永不落库 → 每次调用重新取当前值当峰值。
    后果：输赢家锁利分支 `(最高浮盈 − 当前浮盈 ≥ 8pp)` 恒为 0 → **死代码**。
    铁证：V4.10 回测 41 次"进入赢家宽管"，日志里"最高"永远等于"当前"（130.0/130.0、118.2/118.2），
    且"赢家锁利"出场 0 次。改为显式缺省判定（`is None or >`）。
  ★V4.11 P1-1（误导性日志）★ 避险档初始化行仍打印 V4.8 的"半仓减速，非全停"，
    与 V4.10 起的 halt 实际行为矛盾 → 按实际生效语义打印（含闸门指数名）。
    ...
    for code, px in pm.items():
        ...
        if cost and WINNER_EXEMPT_PCT > 0:
            ...
            if code in g.winner_set and g.hold_days.get(code, 0) <= WINNER_MAX_HOLD:
                is_winner = True
                # ★V4.11★ P0-1 修复★ 原写法 `_pk_prev = g.winner_peak_pnl.get(code, _pr_now)`
                #   把"缺失时默认值"取成了**当前浮盈**，于是首次进入时 `_pr_now > _pk_prev`
                #   恒为 False → 键永远不落库 → 每次调用都重新取到 `_pr_now` 当峰值。
                #   后果：g.winner_peak_pnl 恒等于"当前浮盈"，③b 的 (最高−当前) 恒为 0，
                #   **赢家锁利分支成为死代码**（V4.10 回测 41 次进入赢家宽管、锁利 0 次，
                #   日志里"最高"永远等于"当前"就是铁证）。改为显式缺省判定。
                _pk_prev = g.winner_peak_pnl.get(code)
                if _pk_prev is None or _pr_now > _pk_prev:
...
def initialize(context):
    ...
    log.info("[初始化] V4.11 启动 TRADE_ENABLED={} 调度={} 板块池={}".format(
    ...
    # ★V4.11★ 本版变更指纹（纯 bug 修复，不改策略参数 —— 便于下次回测干净隔离"赢家锁利被激活"的效果）
    log.info("[指纹] ★V4.11★ 仅修 bug、未改参数 → "
             "①修复赢家峰值追踪（g.winner_peak_pnl 缺省值曾取当前浮盈，导致峰值永不落库、"
             "赢家锁利分支成死代码；现改为显式缺省判定）；"
             "②修正过期的避险档初始化日志（原仍打印 V4.8「半仓减速」，与 halt 实际行为矛盾）；"
             "③赢家锁利门槛仍为自最高浮盈回落≥{:.0%}pp".format(WINNER_GIVEBACK_PCT))
    ...
    # ★V4.11★ P1-1 修复★ 此前该行仍打印 V4.8 的"半仓减速，非全停"描述，
    #   与 V4.10 起的 halt 实际行为**互相矛盾** —— 这种"日志说 A、代码做 B"正是
    #   "版本误判/结论误读"的温床。按实际生效语义打印。
    _ro_mode = str(REGIME_RISK_OFF_MODE).lower()
    if _ro_mode.startswith("halt"):
        _ro_desc = "→ 当日【停建仓】（仅停新建仓，持仓管理不受影响）"
    else:
        _ro_desc = "→ 单票仓位×{:.2f}、每日新建上限 {} 笔（半仓减速，非全停）".format(
            REGIME_RISK_OFF_SCALE, REGIME_RISK_OFF_MAX_NEW)
    log.info("[初始化] ★V4.10/V4.11 市场避险档★ 闸门指数={} 指数<MA20（或 MA{}<MA20 且收盘<快线）{}".format(
        REGIME_INDEX or INDEX_FOR_REGIME, REGIME_RISK_OFF_MA_FAST, _ro_desc))
