...
版本：★国金PTrade V1.2★（与山西PTrade_三层量化策略.py 同逻辑，仅券商适配）

★V1.2 修复（首跑「真下单模式」回测暴露，见《回测复盘_山西PTrade_20260922.md》D9~D13）：
...
★V1.2 修复（对应《回测复盘_山西PTrade_V1.2_20260922.md》D16~D21，30万本金真下单模式实测）：
...
def _cur_price(code, data):
    ...
    if data is not None:
        try:
            ...
            pass
    # ★V1.2 D20：移除 get_snapshot 回测噪音（平台在回测模式下调用 get_snapshot 会刷
...
def _record_turn(reason, round_trip=False):
    ...
    if round_trip:
        # ★V1.2 D17：L3 日内T 单独记账，不再污染 L2 年度换手预算；用「发生日」入日志，
...
def _turnover_ok():
    """L2 波段换手是否还在预算内（★V1.2 D17 改为滚动 250 日窗口）。
...
def _do_sell(code, shares, reason):
    ...
    ★V1.2：改走统一委托出口（盘前入队、盘中执行）；被拒返回 0，调用方据此跳过记账。
    """
    cd = _canon(code)
    poss = _get_positions()
    pos = poss.get(cd)
...
def _do_buy_value(code, target_value, reason="buy"):
    ...
    五道约束：断路器 / 在途委托 / 单票上限 / 行业上限 / （★V1.2）整手可行性 + 可用现金。
    返回规划并成功提交（或入队）的市值；0 表示被拦下。
    ★V1.2：任何一项预检不通过都返回 0 并说明原因，**绝不把 0 股委托丢给平台**。
    """
    cd = _canon(code)
...
def _submit_order(code, target_value, reason="trade", side="buy"):
    """统一委托出口（★V1.2 唯一出口，禁止在回调里直接调 order_target_value）。
    ...
    try:
        ...
        return None
    if oid is None:
        g["order_rejected"] = g.get("order_rejected", 0) + 1   # ★V1.2 D19 真实拒单数
        return None
    g["order_submitted"] = g.get("order_submitted", 0) + 1     # ★V1.2 D19 真实下单成功数
    return oid
...
def _mark_bought(code, reason):
    ...
    # ★V1.2 D16：记录持仓归属层级，出场规则据此过滤（而非「标的是否在某个池」）。
...
def _flush_pending():
    """★V1.2 交易执行：出队执行盘前算好的委托（每交易日首个可交易 tick 执行一次）。"""
    pend_raw = g.get("pending_orders", [])
    g["pending_orders"] = []
    if not pend_raw:
        return
    # ★V1.2 D21：同标的同日多笔委托合并（取末笔绝对目标市值）。order_target_value 以绝对
...
def _redistribute(plan, total, layer="L1"):
    """★V1.2 资金再分配：把「预算买不起 1 手」的标的预算顺延给买得起的标的。
...
def _capital_fit_report(total):
    """★V1.2 资本适配自检：启动即暴露「结构性买不起」与「组合最大可落实仓位」。
...
def initialize(context):
    ...
    g["trade_days"] = 0
    g["last_reb_day"] = -999
    # g["ytd_turns"] 已弃用(V1.2)：改用 turn_log_l2 / turn_log_l3 滚动窗口统计
    g["peak_total"] = 0.0
    ...
    g["owner_layer"] = {}         # ★V1.2 cd -> "L1"/"L2"：持仓归属层级（D16 防 L2 规则误清 L1 底仓）
    g["turn_log_l2"] = []        # ★V1.2 L2 回合发生日(trade_days)，_turnover_ok 按 250 日滚动窗口裁剪（D17）
    g["turn_log_l3"] = []        # ★V1.2 L3 日内T 回合发生日，独立计数（D17 不与 L2 共享年度预算）
    g["order_submitted"] = 0     # ★V1.2 实际下发平台的委托数（D19 修正统计口径）
    g["order_rejected"] = 0      # ★V1.2 平台返回 None 的拒单数（D19）
    log.info("[初始化] ★国金PTrade V1.2★ 三层策略 | 标的%d | L1=%.0f%% L2=%.0f%% | TRADE=%s" %
...
def before_trading_start(context, data):
    ...
    # ---- ★V1.2：盘前只算信号、不下单；委托入队等 09:31 执行 ----
    ...
    if total <= 0:
        return
    _capital_fit_report(total)   # ★V1.2 启动自检（只打一次）
...
def _l1_rebalance(total):
    ...
    # ★V1.2 资金再分配：买不起 1 手的标的预算顺延给买得起的标的，避免名义仓位虚高
...
def _daily_risk_scan():
    ...
    for cd, pos in poss.items():
        code = pos["code"]
        # ★V1.2 D16：只处理「确实由 L2 建仓」的持仓；L1 核心仓即使标的在 L2_SET 也跳过。
...
def handle_data(context, data):
    ...
    g["cur_data"] = data          # ★V1.2：供 _ref_price 取盘中价

    # ---- ★V1.2 交易执行：每日首个可交易 tick 出队执行盘前算好的委托 ----
...
def _intraday_hard_stop(data):
    ...
    for cd, pos in poss.items():
        code = pos["code"]
        # ★V1.2 D16：仅 L2 卫星仓参与日内硬止损；L1 核心仓长持不受日内波动干扰
        if g.get("owner_layer", {}).get(cd) != "L2":
            continue
        if _held_days(cd) < 1:
...
def _t_once(code, data, now):
    """L3 日内T 单次触发（★V1.2 D18：统一走 _submit_order 出口，享拒单/熔断/统计一致处理）。

    ★V1.2 D17：开新 T 不再受 L2 年度换手预算(_turnover_ok)约束——L3 日内T 为高频闭环，
...
def _t_force_close(data):
    """14:50 强制回补未平 T（★V1.2 D18：统一走 _submit_order 出口）。"""
...
def after_trading_end(context, data):
    ...
    # ★V1.2 D19：委托口径改为「实际下发平台 / 被拒 / 有效」，不再用 STATS 记账次数冒充
