...
## 十二、v1.7 修复记录（2026-09-24，彬哥哥拍板「方案A」）

8 个月回测复盘（详见 `D:\投研工作台\06-投资策略分析\策略回测\2026-09-24_热点波段策略_8个月回测复盘.md`）暴露尾部风险：688019 一笔 **-24.8%**（科创板 20cm 波动 + 隔夜跳空击穿 5% 止损，限价单救不回跳空）、601600 -14.8%（跌停开盘）。两笔约占全部亏损四成。彬哥哥在 A（剔除688）/ B（688减仓）/ C（688放宽止损）三案中选定 **方案A：剔除科创板**。

改动（`hotspot_trader_v1_ptrade.py` v1.7）：
1. **新增 `EXCLUDE_STAR = True`**：过滤链第 5 步首位剔除 `688` 开头标的（ST 判定之前），WATCHLIST 里的科创板票全部不再进候选。
2. **时间止损日志补盈利%**：`时间止损 盈利=%.1f%% 持仓=%d日`——修复复盘里 002050 只见天数不见亏多少的问题。
3. 头部版本注释更新至 v1.7。

验证：py_compile OK；离线验证器 ALL PASS。

⚠️ 顺带提醒：回测撮合比例 0.25 会低估落袋收益（301068 拆单到次日更差价），建议在回测设置里调到 1.0 重跑取干净读数——平台面板设置，无需改代码。
