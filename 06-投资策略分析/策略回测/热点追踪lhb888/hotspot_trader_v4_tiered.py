...
v4.1 相对 v4.0：修日志（初始化行版本号、时间止损打印实际生效天数与档位、新增 [分层] 定档日志）；
  ...
  依据：真实回测账本（2026-05-06~06-30）22 只已平仓票单票 +8.64%、胜率 68.2%，
        ...
        赢家回吐是最大改进空间。按 T+1 收盘浮盈分层后机会差异达 4 倍：
          T+1 <  0%  MFE  +7.42%  实现 -4.77%
          T+1 >  5%  MFE +30.23%  实现 +14.28%
        两者却共用同一套出场参数 -> 强票被过早减半、弱票被持有过久。
  做法：强启动(T+1>=+5%) 放宽减半/回撤阈值让利润奔跑；弱启动(T+1<0%) 缩短持有。
  未动：TP_HALF / HALF_DRAWDOWN 的全局值（v3.4 试过放宽、v3.5 已回退，勿重复）；
        硬止损分板 4%/7%；动态保本 3%。
  警告：本改动【未经样本外验证】，样本仅 35 笔。TIERED_EXIT 置 False 即完全回退 v3.0 行为。
...
def _exit_param(name, base_val, board, tier, default):
    """v4.0 分层出场取参：tier 有覆盖则用覆盖，否则回退基线（分板 dict / 标量通用）"""
    if not TIERED_EXIT or tier == "base":
        v = base_val
    else:
        v = EXIT_TIER_OVERRIDE.get(tier, {}).get(name, base_val)
    return v.get(board, default) if isinstance(v, dict) else v


def _tier_of(context, code, held_days, rt):
    """v4.0：用 T+1 收盘浮盈定档（只记录一次，之后固定，避免盘中抖动改档）"""
    t = getattr(context, "tier", None)
    if t is None:
        t = {}
        context.tier = t
    if code in t:
...
def monitor_risk(context, data=None):
    ...
    for code, px in pm.items():
        ...
        _tier = _tier_of(context, code, held_days, rt) if TIERED_EXIT else "base"
        ...
        hard = _exit_param("HARD_STOP_PCT", HARD_STOP_PCT, board, _tier, 0.04)
        ...
        trail = _exit_param("TRAIL_PCT", TRAIL_PCT, board, _tier, 0.08)
        guard = _exit_param("TRAIL_GUARD", TRAIL_GUARD, board, _tier, 0.03)
        ...
        dd = _exit_param("HALF_DRAWDOWN", HALF_DRAWDOWN, board, _tier, 0)
        ...
        tp = _exit_param("TP_HALF_PCT", TP_HALF_PCT, board, _tier, 0)
    ...
    for c in list(halved.keys()):
        if c not in pm:
            ...
            _t = getattr(context, "tier", None)
            if _t is not None and c not in pm:
                _t.pop(c, None)
